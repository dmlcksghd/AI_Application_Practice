...
class PPOAgent(object):
    ...
    def __init__(self, make_env, args):
        ...
        self.entropy_coef_decay_step = (args.entropy_coef - 0.005) / entropy_coef_decay_rollouts
        ...
        # opponent agent nets
        self.encoder_op = CnnEncoder().apply(init_weights).to(self.device)
        self.actor_op = ContinuousActor(self.encoder_op, self.device).apply(init_weights).to(self.device)
        self.critic_op = Critic(self.encoder_op, self.device).apply(init_weights).to(self.device)

        self.actor_op.load_state_dict(torch.load(
            #'C:/Users/theho/git_AI_Application_Practice/Olympic/train_history/olympics-running/11_26_0_47/actor.pth',
            'C:/Users/theho/git_AI_Application_Practice/Olympic/train_history/olympics-running/12_8_22_28/actor_solved.pth',
            map_location=self.device))
        self.critic_op.load_state_dict(torch.load(
            'C:/Users/theho/git_AI_Application_Practice/Olympic/train_history/olympics-running/12_8_22_28/critic_solved.pth',
            map_location=self.device))
        self.encoder_op.load_state_dict(torch.load(
            'C:/Users/theho/git_AI_Application_Practice/Olympic/train_history/olympics-running/12_8_22_28/encoder_solved.pth',
            map_location=self.device))
    ...
    def _get_opponent_action(self, state):
        ...
        action, dist = self.actor_op(state)

        return list(action.detach().cpu().numpy()).pop()

    def _step(self, action: float, action_opponent: float):
        """
        Make action in enviroment chosen by current policy,
        if not evaluate - collect elements of trajectory.
        """
        # print(action)
        next_state, reward, terminated, truncated, _ = self.env.step(action, action_opponent)
    ...
    def train(self):
        ...
        score = 0
        state, state_op = self.env.reset()
        state = np.asarray(state)
        state_op = np.asarray(state_op)
        # print(f"state:{state}")
        # exit()
        self.num_episode = 0
        self.time_step = 0
        ...
        for step_ in range(self.total_rollouts):
            for _ in range(self.rollout_len):
                ...
                opponent_action = self._get_opponent_action(state_op)
                next_state, reward, done = self._step(action, opponent_action)
                state = next_state
                score += reward[0][0]
            ...
            if step_ < self.total_rollouts * self.entropy_coef_decay_rollouts:
                self.entropy_coef *= max(np.exp(-self.entropy_coef_decay_step), 0.005)
    ...
    def _update_weights(self):
        ...
        ratio = sum(ratio_list) / len(ratio)
