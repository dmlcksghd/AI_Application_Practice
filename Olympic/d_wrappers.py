...
class CompetitionOlympicsEnvWrapper(gym.Wrapper):
    ...
    def __init__(self, env, agent=None, args=None):
        ...
        self.max_episode_steps = 400  # 에피소드당 최대 스텝 수를 설정
        self.negative_reward_per_step = -0.005  # 스텝마다 받을 음의 보상을 설정

    def reset(self):
        self.episode_steps = 0
        observation = self.env.reset()

        self.current_step = 0  # 현재 스텝 카운터를 초기화
        ...
        return [observation_controlled_agent], [observation_opponent_agent]
    ...
    def step(self, action_controlled, action_opponent):
        ...
        self.total_steps += 1
        self.current_step += 1  # 현재 스텝 카운터를 증가
        ...
        ################################
        # 에이전트가 제한된 스텝 동안 스텝마다 음의 보상을 받도록 합니다.
        negative_reward = self.negative_reward_per_step
        ...
        adjusted_reward = self.adjust_reward(next_observation_controlled_agent, reward_controlled) + negative_reward

        info = {}

        # 에이전트가 지정된 시간 제한 내에 결승선을 통과하지 못한 경우 확인합니다.
        if self.current_step >= self.max_episode_steps:
            done = True  # 에피소드 종료
            not_passed_finish_line_penalty = -5.0  # 결승선을 통과하지 못한 경우 음수 보상을 설정합니다.
            adjusted_reward += not_passed_finish_line_penalty
    ...
    def adjust_reward(self, observation, original_reward):
        ...
        if hasattr(self, 'prev_farthest_4') and self.prev_farthest_4 is not None:
            agent_position = np.where(observation == 10)
    ...
    def isRightArrow(self, array, target_value=10):
